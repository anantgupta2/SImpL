...
from src.utils.oat_prompt_templates import implication_prompt, maybe_apply_chat_template, qa_eval_prompt, qa_cot_prompt
...
@dataclass
class ImplicationsOnlyArgs(PPOArgs):
    prompt_data: str = ""
...
def configure_implications_only_args(args: ImplicationsOnlyArgs) -> ImplicationsOnlyArgs:
...
class ImplicationsOnlyActor(PPOActor):
    ...
    def step(
        ...
        for doc_idx, article in enumerate(prompts):
            prompt_text = implication_prompt(article, self.reasoning_max_items)
        ...
        logging.info(
            "Implications-only actor done: docs=%d transitions=%d per_doc=%d",
            len(prompts),
            len(trajectory_data),
...
class ImplicationsOnlyLearner(PPOLearner):
    def _init(self, args: ImplicationsOnlyArgs, actors: List[ActorBase]) -> None:
...
def run_implications_only_oat(args: ImplicationsOnlyArgs):
    args = configure_implications_only_args(args)
    args = default_args_validation(args)

    program, local_resources = get_program(
        args,
        learner_cls=ImplicationsOnlyLearner,
        actor_cls=ImplicationsOnlyActor,
    )
    lp.launch(
        program,
...
if __name__ == "__main__":
    cli_args: ImplicationsOnlyArgs = get_default_args(ImplicationsOnlyArgs)
    run_implications_only_oat(cli_args)