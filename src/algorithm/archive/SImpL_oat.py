...
Improvements over the earlier combined CoT+understanding trainer (removed):
...
class SImpLArgs(PPOArgs):
    ...
    # CoT:understanding mixture ratio (per doc). Default = 1 understanding row + N cot
    # rows (one per question) -> cot-heavy ~N:1. Increase understanding_rows_per_doc to
    # replicate the understanding task (up-weight understanding); set cot_rows_per_doc>0
    # to cap the number of cot rows (down-weight cot). Ratio = cot_rows : understanding_rows.
    understanding_rows_per_doc: int = 1
    cot_rows_per_doc: int = 0  # 0 = one cot row per question (all)

    # If True, use the model's ENTIRE understanding generation as the understanding
    # (instead of strictly extracting the <understanding>...</understanding> span and
    # DROPPING any generation without tags). Base models often omit tags, so strict
    # extraction silently discards good understandings; this never wastes one.
    ...
    # If True, the QA-from-understanding step answers DIRECTLY (no chain-of-thought,
    # just the boxed letter). Forces the understanding to carry the reasoning, which
    # makes the reward a sharper signal of understanding quality. Pair with a small
    # qa_eval_max_tokens (e.g. 16 letter-only, 40 tiny-budget).
    qa_direct_answer: bool = False
...
class SImpLActor(PPOActor):
    def init(self, actor_id, save_path):
        ...
        self.qa_direct_answer = bool(getattr(self.args, "qa_direct_answer", False))
    ...
    def _extract_understanding_from_tags(self, text: str) -> str:
        # Ablation: use the whole generation as the understanding (never drop).
        if getattr(self, "use_full_understanding_output", False):
            return text.strip()
    ...
    def prepare_data(self, strategy, tokenizer):
        ...
        def flatten_and_mix_questions(batch):
            ...
            for article, qs_json in zip(batch[input_key], batch[output_key]):
                ...
                if not valid_qs:
                    continue
                # Mixture ratio knobs: replicate the understanding task and/or cap cot rows.
                # cot_rows_per_doc: 0 -> one cot row per question (all); >0 -> cap to that
                # many; <0 -> ZERO cot rows (understanding-only training).
                u_rows = max(1, int(getattr(self.args, "understanding_rows_per_doc", 1)))
                cot_cap = int(getattr(self.args, "cot_rows_per_doc", 0))
                if cot_cap < 0:
                    cot_qs = []
                elif cot_cap == 0:
                    cot_qs = valid_qs
                else:
                    cot_qs = valid_qs[:cot_cap]
                # One (or more) understanding task(s), each covering all questions of the doc...
                for _ in range(u_rows):
                    new_articles.append(article)
                    new_questions.append(json.dumps({"task_type": "understanding", "questions": valid_qs}))
                # ...and one cot task per (kept) question.
                for vq in cot_qs:
                    new_articles.append(article)
