...
while true; do
  ...
  room=$(( CAP - inflight ))
  left=$(DRY_RUN=1 python scripts/claude/launch_transfers.py 2>/dev/null | head -1)
  ...
  if (( room > 0 )); then
    MAX_SUBMIT="$room" python scripts/claude/launch_transfers.py 2>&1 | grep -E "^\[OK|^\[FAIL|^\[skip|to submit" || true
  else
